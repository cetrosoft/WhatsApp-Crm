/**
 * CRM Module Migration - FIXED TABLE ORDER
 *
 ...
 * Tables ordered by dependencies to avoid foreign key errors
 */
...
CREATE TABLE IF NOT EXISTS contacts (
  ...
  phone TEXT NOT NULL,
  email TEXT,
  whatsapp_id TEXT, -- For future WhatsApp integration
  avatar_url TEXT,
...
CREATE TABLE IF NOT EXISTS segments (
  ...
  -- Example: {"rules": [{"field": "status", "operator": "equals", "value": "lead"}], "logic": "AND"}
  filter_criteria JSONB NOT NULL DEFAULT '{"rules": [], "logic": "AND"}',

  -- Dynamic segments auto-update, static segments are snapshots
...
CREATE TABLE IF NOT EXISTS segment_members (
  ...
  entity_id UUID NOT NULL, -- contact_id or company_id
...
CREATE TABLE IF NOT EXISTS deals (
  ...
  -- For drag-drop ordering within a stage
  stage_order INTEGER,
...
CREATE TABLE IF NOT EXISTS interactions (
  ...
  -- Interaction Details
  user_id UUID REFERENCES users(id) ON DELETE SET NULL, -- Who logged it
...
-- Function to get contact count by status for an organization
...
COMMENT ON TABLE companies IS 'CRM companies (organizations) - created first to avoid circular dependencies';
COMMENT ON TABLE contacts IS 'CRM contacts (individuals) linked to companies with multi-tenant isolation';
